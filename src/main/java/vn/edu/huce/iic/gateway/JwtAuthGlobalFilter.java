...
/**
 * Cổng vào duy nhất: chặn đường nội bộ, gắn {@code X-Request-Id}, và xác thực access token JWT cục bộ
 * (không gọi backend) trước khi chuyển tiếp. Header {@code Authorization} được giữ nguyên để backend
 * tự xác thực lại; phân quyền dữ liệu vẫn là việc của service.
 */
@Component
public class JwtAuthGlobalFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        // Preflight CORS do backend trả lời; không có Authorization nên không thể yêu cầu JWT.
        ...
        // Ghi đè X-Request-Id do client gửi để không giả mạo được id truy vết.
    ...
    /** @return {@code null} nếu token hợp lệ, ngược lại là lý do (chỉ để log, không trả cho client). */
    private String verify(String authorization, ServerHttpRequest request) {
        ...
        try {
            Claims claims = Jwts.parser().verifyWith(key).build()
                    ...
                    .getPayload();
            if (!LOAI_ACCESS.equals(claims.get(CLAIM_LOAI, String.class))) {
            ...
            if (!hasScope) {
                return requireScope ? "missing scope" : null;
            }
    ...
    /** Chạy trước mọi filter định tuyến. */
    @Override
    public int getOrder() {
        return -100;
    }
